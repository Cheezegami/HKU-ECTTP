...
class GenericObject(object) :
    ...
    def checkMousePosition(self):
        if(mousePressed == True):
            if(mouseX >= self.int_oX and mouseX <= self.int_oX + self.int_oWidth and mouseY >= self.int_oY and mouseY <= self.int_oY + self.int_oHeight):
                ScoreBoard.int_score += 20
                self.alterObjectPosition()

    def alterObjectPosition(self):
        self.int_oX = self.int_border+random(width-self.int_border)
        self.int_oY = self.int_border+random(height-self.int_border)
        self.int_oWidth = random(self.int_minWidth,self.int_maxWidth)
        if(self.int_movestyle == 0):
            int_randomizedDirection = random(4)
            if(int_randomizedDirection == 0):
                self.bool_oTurnLeft = True
                self.bool_oTurnUp = True
            elif(int_randomizedDirection == 1):
                self.bool_oTurnLeft = True
                self.bool_oTurnUp = False
            elif(int_randomizedDirection == 2):
                self.bool_oTurnLeft = False
                self.bool_oTurnUp = False
            elif(int_randomizedDirection == 3):
                self.bool_oTurnLeft = False
                self.bool_oTurnUp = True
        if(self.base_oWidth == self.base_oHeight):
            self.int_oHeight = self.int_oWidth
        else:
            self.int_oHeight = random(self.minWidth,self.maxWidth)
